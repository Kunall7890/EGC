...
function getCounterFilePath(sessionId) {
  return path.join(os.tmpdir(), `egc-tool-count-${sessionId}`);
}
...
function runTests() {
  ...
  if (test('does NOT suggest compact before threshold', () => {
    ...
    const result = runCompact({ EGC_SESSION_ID: sessionId, COMPACT_THRESHOLD: '5' });
    assert.ok(
  ...
  if (test('ignores non-numeric COMPACT_THRESHOLD', () => {
    ...
    cleanup();
    fs.writeFileSync(counterFile, '49');
    const result = runCompact({ EGC_SESSION_ID: sessionId, COMPACT_THRESHOLD: 'abc' });
  ...
  if (test('uses separate counter files per session ID', () => {
    ...
    try {
      runCompact({ EGC_SESSION_ID: sessionA });
      runCompact({ EGC_SESSION_ID: sessionA });
      runCompact({ EGC_SESSION_ID: sessionB });
  ...
  if (test('rejects COMPACT_THRESHOLD=0 (falls back to 50)', () => {
    ...
    cleanup();
    fs.writeFileSync(counterFile, '49');
    const result = runCompact({ EGC_SESSION_ID: sessionId, COMPACT_THRESHOLD: '0' });
  ...
  if (test('accepts COMPACT_THRESHOLD=10000 (boundary max)', () => {
    ...
    const result = runCompact({ EGC_SESSION_ID: sessionId, COMPACT_THRESHOLD: '10000' });
  ...
  if (test('rejects COMPACT_THRESHOLD=10001 (falls back to 50)', () => {
    ...
    cleanup();
    fs.writeFileSync(counterFile, '49');
    const result = runCompact({ EGC_SESSION_ID: sessionId, COMPACT_THRESHOLD: '10001' });
  ...
  if (test('rejects float COMPACT_THRESHOLD (e.g. 3.5)', () => {
    ...
    cleanup();
    fs.writeFileSync(counterFile, '49');
    const result = runCompact({ EGC_SESSION_ID: sessionId, COMPACT_THRESHOLD: '3.5' });
