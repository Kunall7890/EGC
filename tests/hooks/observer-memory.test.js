...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'egc-observer-test-'));
}
...
test('analysis temp file is created and cleaned up', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  assert.ok(content.includes('egc-observer-analysis'), 'Should create a temp analysis file');
...
test('observer-loop uses project-local temp directory for analysis artifacts', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  assert.ok(content.includes('observer_tmp_dir="${PROJECT_DIR}/' + (process.env.EGC_ARTIFACTS_DIR_NAME || '.observer-tmp') + '"'), 'Should keep observer temp files inside the project');
  assert.ok(content.includes('mktemp "${observer_tmp_dir}/egc-observer-analysis.'), 'Analysis temp file should use the project temp dir');
  assert.ok(content.includes('mktemp "${observer_tmp_dir}/egc-observer-prompt.'), 'Prompt temp file should use the project temp dir');
});

test('observer-loop loads prompt content before invoking egc', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  assert.ok(content.includes('prompt_content="$(cat "$prompt_file" 2>/dev/null || true)"'), 'Prompt should be read into memory before the egc invocation');
  assert.ok(content.includes('-p "$prompt_content"'), 'Gemini should receive the in-memory prompt content');
  assert.ok(!content.includes('-p "$(cat "$prompt_file")"'), 'Gemini should not depend on re-reading the prompt file during invocation');
});
...
test('prompt references analysis_file not full OBSERVATIONS_FILE', () => {
  ...
  assert.ok(heredocEnd > heredocStart, 'Should find prompt heredoc end');
  const promptSection = content.substring(heredocStart, heredocEnd);
  assert.ok(promptSection.includes('${analysis_relpath}'), 'Prompt should point Gemini at the sampled analysis file (via relative path), not the full observations file');
});

test('observer-loop wait helper retries SIGUSR1-interrupted waits while egc child is alive', () => {
  if (process.platform === 'win32') {
    return;
  }

  const content = fs.readFileSync(observerLoopPath, 'utf8');
  const helperMatch = content.match(/wait_for_egc_analysis\(\) \{[\s\S]*?\n\}/);
  assert.ok(helperMatch, 'observer-loop.sh should define wait_for_egc_analysis helper');
  ...
  const script = [
    ...
    'signaler=$!',
    'wait_for_egc_analysis "$egc_child"',
    'status=$?',
...
test('observer-loop wait helper preserves real nonzero egc exits', () => {
  if (process.platform === 'win32') {
    return;
  }
...
test('egc invocation includes --allowedTools flag', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  assert.ok(content.includes('--allowedTools'), 'observer-loop.sh should include --allowedTools flag in egc invocation');
});
...
test('egc invocation still includes ECC_SKIP_OBSERVE and ECC_HOOK_PROFILE guards', () => {
  ...
  assert.ok(egcLine, 'Should find egc --model invocation line');
  // The skip guards may be on the same line or the line just above the egc command.
  const egcLineIndex = lines.indexOf(egcLine);
  const fullCommand = lines.slice(Math.max(0, egcLineIndex - 2), egcLineIndex + 3).join(' ');
  assert.ok(fullCommand.includes('ECC_SKIP_OBSERVE=1'), 'egc invocation should include legacy ECC_SKIP_OBSERVE=1 guard');
  assert.ok(fullCommand.includes('ECC_HOOK_PROFILE=minimal'), 'egc invocation should include legacy ECC_HOOK_PROFILE=minimal guard');
  assert.ok(fullCommand.includes('EGC_SKIP_OBSERVE=1'), 'egc invocation should include canonical EGC_SKIP_OBSERVE=1 guard');
  assert.ok(fullCommand.includes('EGC_HOOK_PROFILE=minimal'), 'egc invocation should include canonical EGC_HOOK_PROFILE=minimal guard');
});
