...
function runTests() {
  ...
  if (test('uses custom min_session_length from config file', () => {
    ...
    // The config path is: path.join(__dirname, '..', '..', 'skills', 'ai', 'continuous-learning', 'config.json')
    // __dirname = scripts/hooks, so config = repo_root/skills/ai/continuous-learning/config.json
  ...
  console.log('\nRound 53: GEMINI_TRANSCRIPT_PATH fallback:');

  if (test('falls back to GEMINI_TRANSCRIPT_PATH env var when stdin is invalid JSON', () => {
    const testDir = createTestDir();
    ...
    const result = spawnSync('node', [evaluateScript], {
      ...
      input: 'invalid json {{{',
      timeout: 10000,
      env: { ...process.env, GEMINI_TRANSCRIPT_PATH: transcript }
    });
  ...
  if (test('falls back to defaults when config file contains invalid JSON', () => {
    ...
    //   path.join(__dirname, '..', '..', 'skills', 'ai', 'continuous-learning', 'config.json')
    // where __dirname = scripts/hooks/ → config = repo_root/skills/ai/continuous-learning/config.json
    const configPath = path.join(__dirname, '..', '..', 'skills', 'ai', 'continuous-learning', 'config.json');
